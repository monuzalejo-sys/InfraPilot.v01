...
## 2b. Referencias: se combinan MECANISMOS, nunca contenido — y siempre con versión móvil
...
1. **Lee `_INDEX.md` antes de escribir el prompt**, justo después de la memoria (§0).
   Si el dueño entrega una referencia nueva, primero la FICHAS con la plantilla (qué
   aporta / qué NO se copia / traducción al negocio / riesgos / historial) y la
   agregas al índice. Sin ficha no hay combinación.
2. **Elige 2–3 referencias por landing, una por zona** (hero, producto/carta, cierre).
   Tres referencias peleando por el hero = ruido. Del TOONHUB (ficha 01) se toma el
   carrusel con fondo que cambia de color; jamás su marca, sus textos ni sus figuras.
3. **Cada mecanismo tomado se justifica en términos de venta** dentro del prompt; si
   no ayuda a que el cliente pida, se cae.
4. **MÓVIL ES CANAL PRINCIPAL (WhatsApp, campañas, QR): toda combinación trae su
   adaptación móvil explícita**, nunca implícita: qué se esconde, qué se apila, qué
   escala cambia, qué gesto reemplaza al hover (swipe/tap), tap targets ≥40 px,
   alturas en `100dvh`, peso de imágenes (≤150 KB, WebP/AVIF con alfa),
   `prefers-reduced-motion` con fallback, y qué se mide a 390×844 con Edge headless
   por CDP (KN-003). Si una referencia es de escritorio y no se puede adaptar sin
   perder su gracia, no se usa.
5. **Aprendes por canal:** al registrar el veredicto (§4) anotas la combinación (qué
   fichas, en qué zona), el veredicto del dueño EN MÓVIL y EN ESCRITORIO por separado
   y el porqué; actualizas la columna "Resultados" del índice y el "Historial de uso"
   de cada ficha. Una combinación que gustó en escritorio y falló en móvil es una
   lección de móvil, no un éxito.

## 3. La plantilla que produces

Un solo bloque copiable, en español, en segunda persona dirigido a quien diseña.
Doce apartados; ninguno se omite (si uno no aplica, dilo y explica por qué):
...
11. **Referencias combinadas** — qué ficha de la biblioteca (§2b) manda en cada zona
    (hero / producto / cierre), qué mecanismo exacto se toma de cada una y por qué vende.
12. **Adaptación móvil** — la versión a 390 px de cada mecanismo del apartado 11 (qué
    se apila o esconde, gestos, escalas, pesos, reduced-motion) y qué se mide.
...
lección de memoria aplicaste, y qué fichas de referencia combinaste (con su adaptación
móvil). Si las fuentes son tan pobres que el prompt sería una
