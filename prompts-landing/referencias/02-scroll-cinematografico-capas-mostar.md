# 02 · Scroll cinematográfico por capas con "escenas" ("Mostar city")

- **Recibida:** 2026-08-23 · **Fuente:** especificación en texto entregada por el dueño · **Stack original:** vanilla (`index.html` + `styles.css` + `script.js`), sin frameworks ni build, assets remotos
- **Para qué la dio el dueño:** referencia para combinar en landings ("te voy a dar varias referencias para que empieces a combinarlas"), canal principal móvil.

## Qué aporta (mecanismos reutilizables)
- **Composición:** una sola "escena" pegada (`position: sticky; height: 100vh`) dentro de un rail de scroll largo (`100vh + 3700px`). Capas PNG con borde transparente (cielo → resplandor → fondo medio → marco partido izq/der → puente en primer plano → close-up) apiladas por z-index; el scroll no desplaza la página, **dirige una película**: título que sube y se desvanece, el primer plano se abre en dos mitades (splitframe), entra un close-up, luego un panel de historia con dos datos grandes, luego otro, y al final un **slider infinito de tarjetas** que entra desde la derecha contra-escalado para que las tarjetas se vean reales mientras el fondo sigue haciendo zoom.
- **Movimiento / interacción:** TODO se deriva de una sola variable (`smoothScroll`, suavizada con `lerp 0.14`) y del puntero (`lerp 0.12`); segmentos `segmentInOut(a,b,c,d)` con `smoothstep` dan entrada/salida por tramo; escritura de ~45 custom properties por frame (la CSS no sabe de JS: solo lee `var(--x)`); `requestAnimationFrame` con guardia `rafPending` y re-solicitud mientras haya delta; parallax con el puntero (`--mx/--my`) en todas las capas; blur + brightness + velo degradado (`--blur-tint`) para "enfocar" la escena activa; slider infinito con 3 copias de las tarjetas y salto sin transición (`.is-jumping`) al normalizar; `prefers-reduced-motion` anula inercia y parallax pero mantiene el scrub.
- **Color:** fondo celeste frío `#79b7dd` + crema `#fdf1e1` + tinta `#111411`; velo azul `74,181,224`. Lo reutilizable es el **velo de color que "enfoca"** y las tarjetas crema sobre escena — no la paleta (POL-001: cálida; el azul se sustituye).
- **Tipografía / copy:** display serif (`Ogg Medium`) enorme como título de escena (`14rem` → `4.5rem` móvil) y para cifras grandes (`dt` 4.2rem); sans para cuerpo; píldoras crema como etiquetas; panel de historia = h2 + párrafo + 2 datos (año / hecho). Patrón "un dato grande + una línea" es directamente útil para comida: "2 sedes / Villa del Viento y Vía al Bosque", "Se paga al recibir".

## Qué NO se copia
- Mostar, Bosnia, todas las fotos de figma.site/cloudfront, los iconos, los textos, las fechas 1566/2005, la fuente `Ogg Medium` (licencia ajena, URL de un CDN de terceros), el azul `#79b7dd`/`74,181,224`.
- El rail de **3700 px de scroll** tal cual: en móvil es mucho pulgar; se recorta (ver riesgos).

## Cómo se traduce a un negocio real (Villa Broaster u otro)
| Referencia | Proyecto |
|---|---|
| Capas PNG de ciudad | Capas del producto: fondo de mesa/mantel, el pollo entero por tandas, presas en primer plano; las fotos en fondo blanco de `villa-broaster/docs/identidad/fotos-productos/` recortadas a PNG con alfa |
| "MOSTAR" 14rem que sube y se desvanece | "VILLA BROASTER" / "POR TANDAS" |
| Splitframe que se abre | La canasta/bolsa que se abre y deja ver el close-up del pollo (frame-two) |
| Panel historia 1 (puente, 1566/2005) | "Dos sedes": Villa del Viento · Vía al Bosque (zona, sin calles); "Se paga al recibir" |
| Panel historia 2 (bazar) + botón "Open old town notes" | Promos reales (cuando existan) + "Ver la carta" |
| Slider infinito de 5 tarjetas de sitios | Carrusel infinito de productos: kicker = categoría, h3 = nombre, p = 1 línea, pin = foto; tap → agrega o abre el producto. Precio real por sede en la tarjeta |
| Velo azul para enfocar | Velo cálido oscuro (`33,33,33` o rojo muy oscuro) |

## Riesgos al usarla
- **Móvil:** `100vh` pegado + 3700 px de rail ⇒ usar `100dvh`, recortar el rail a ~2000 px y reducir a 3 escenas (hero → sedes → carta); el parallax de puntero no existe en táctil: reemplazar por giroscopio opcional o nada; el slider necesita swipe además de flechas; `backdrop-filter`/blur animado pesa en gama baja ⇒ blur máximo 8 px o velo sin blur cuando `prefers-reduced-motion` o `navigator.hardwareConcurrency <= 4`.
- **Peso:** 7 PNG grandes con alfa; exigir WebP/AVIF con alfa ≤150 KB cada uno y `loading="eager"` solo para las 3 primeras capas.
- **Accesibilidad:** todo el contenido vive en capas absolutas; los paneles deben seguir siendo legibles sin JS (los textos existen en el DOM, bien) y el contraste crema sobre velo se mide.
- **Copy:** la estructura pide "datos grandes"; con KN-001 solo van datos reales (sedes, "se paga al recibir", horario solo si está confirmado).
...
|---|---|---|---|---|
| 2026-08-23 | villa-broaster · prompt v2 — zona **PROMO/CARTA** | fichas 01 (héroe) y 03 (cierre) | **Mixto: sobrevivió lo único que sobrevivió.** Rechazo global en móvil, pero el "dato grande" **sí se ejecutó y no fue criticado** | Solo DOS mecanismos: "dato grande + una línea" (2 sedes · $0 por internet) y las capas contra-escaladas, movidas del rail de scroll al **interior de la tarjeta de promo de 272 px**: al tocarla, las fotos de lo que incluye vuelan y arman el plan. El rail de 3700 px y el parallax de puntero se descartaron por móvil. **Qué pasó:** los bloques **"2 sedes" y "$0"** aparecieron tal cual en la captura de la carta, en display sobre panel oscuro, y son de lo poco que el dueño no señaló — **es el mecanismo más barato y más resistente de la biblioteca: funciona aunque el medio sea estático, porque no depende de gesto ni de tiempo**. En cambio la promo que se arma **quedó invisible**: el estado `promoArmada` existe en el markup, pero en un lienzo quieto nadie lo ve (KN-008). |
| 2026-08-29 | **prommter** · `prommter-agencia-v2.md` — zona **CIFRA** | fichas 01 (héroe) y 03 (cierre) | pendiente (PEND-005) | Se toma **solo** el "dato grande + una línea" —el mecanismo que sobrevivió al rechazo— y se usa **tres veces**, siempre con dato citable: la comisión que se va (`$360.000–$540.000` al mes, `precios-v2.html:205-207`, ámbar sobre noche, cruzada por franja diagonal al 18 %), la velocidad del sistema (`2 toques` para cobrar / `6 toques` para tomar un pedido, `villa-broaster/brief.md:11`) y el portafolio (`4 sistemas · 1 en uso todos los días`). Se descartan explícitamente el rail de 3700 px, la escena pegada por capas y el parallax de puntero: en móvil son demasiado pulgar y aquí no hay fotos que apilar. |
| 2026-08-23 | villa-broaster · prompt **v3** — zona **CARTA/PROMO** | fichas 01 (portada) y 03 (cierre) | pendiente (PEND-004) | Se sube la apuesta por lo que funcionó: el "dato grande" pasa a **cifra gigante en amarillo brasa cruzada por una franja diagonal**, y la promo se dibuja **congelada a medio armar** (piezas en vuelo con sombra e inclinación), no en su estado final ni escondida tras un toque. |
| 2026-08-29 | **prommter** · `prommter-agencia-v3-mejora.md` — zona **ESCENA héroe→calculadora** | fichas 01 (héroe congelado) y 03 (foco que dirige) | pendiente (PEND-006) | Primera vez que se toma el **MOTOR** de esta ficha y no solo su "dato grande": una sola variable de scroll suavizada (`lerp .14`) + tramos `smoothstep(a,b)` escribiendo **≤ 8 custom properties** por cuadro, con guardia `rafPending` y listener `{passive:true}`. Se traduce el "primer plano que se abre en dos" a los cuatro teléfonos que **se abren en abanico** (∓ 8 vw, no ∓ 46 vw: a 390 px más apertura los saca de pantalla) mientras el titular sube −60 px, y la cifra de comisión entra contra-escalada con su franja diagonal barriendo por `clip-path`. Se siguen descartando el rail de 3700 px (aquí ≤ 60 vh extra, medible), las capas PNG, el parallax de puntero y el slider infinito. Justificación de venta: la página **entrega** al visitante a la calculadora en vez de dejar que llegue solo. Presupuesto: fps ≥ 50, ≤ 3 cuadros largos, ≤ 6 animaciones vivas; con `reduce` o `hardwareConcurrency ≤ 4`, escena congelada en su estado final legible. |
